...
export default {
  ...
  methods: {
    fetchAppointments() {
      fetch("https://26mzjui31f.execute-api.us-east-1.amazonaws.com/prod/appointments")
        .then(res => res.json())
        .then(data => {
    ...
    updateStatus(appointment, newStatus) {
      const url = `https://26mzjui31f.execute-api.us-east-1.amazonaws.com/prod/appointments/${appointment.appointmentId}`;
      fetch(url, {
        method: "PATCH",
        ...
        .catch(err => { alert("Update failed. See console for details."); });
    }
  }
};
</script>
